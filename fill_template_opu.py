...
def add_signature_to_cell(cell, sig_b64):
    ...
    try:
        ...
        pass
    tmp = os.path.join(_TMP_DIR, 'sig.png')
...
def fill_template_opu(data_file, output_path, template_path):
    ...
    t0 = doc.tables[0]
    set_cell_text(t0.rows[0].cells[1],  data.get('fecha', ''))
    set_cell_text(t0.rows[1].cells[1],  data.get('tecnico', ''))
    set_cell_text(t0.rows[2].cells[1],  data.get('instalacion', ''))
    set_cell_text(t0.rows[3].cells[1],  data.get('cliente', ''))
    set_cell_text(t0.rows[4].cells[1],  data.get('login', ''))
    set_cell_text(t0.rows[5].cells[1],  data.get('fibra', ''))
    set_cell_text(t0.rows[6].cells[1],  data.get('cable_acometida',''))
    set_cell_text(t0.rows[7].cells[1],  data.get('codigo_caja', ''))
    set_cell_text(t0.rows[8].cells[1],  data.get('hilo', ''))
    set_cell_text(t0.rows[9].cells[1],  data.get('sw', ''))
    set_cell_text(t0.rows[10].cells[1], data.get('puerto', ''))
    set_cell_text(t0.rows[11].cells[1], data.get('responsable', ''))
